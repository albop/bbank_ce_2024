...
$$\pi_{ij} = prob \left( |y_{t+1}-x_j| = \inf_k |y_{t+1}-x_k| \left| y_t=y_i \right. \right)$$
...
$$E(T_N^{MC}) \propto \frac{1}{\sqrt{N}}$$
    - slow
    - on the plus side:  rate [ independent of the dimension of 
]{.underline} $\epsilon$
...
- Gauss quadrature magic
  - a way to choose $\epsilon_i$ __and__ $w_i$ such that $$\left(f\rightarrow\sum_{n=1}^N w_n f(\epsilon_i) \right)= \left.I\right|_{\mathcal{F}_{2N}}$$
...
$$\frac{1}{\sqrt{2 \pi \sigma^2}}\int f(x) e^{-\frac{x^2}{2\sigma^2}}dx = {\frac{1}{\sqrt{2 \pi}}}\int f(u {\sigma}) e^{-\frac{u^2}{2}}du $$
$${\frac{1}{\sqrt{2\pi}}}\sum_n w_n f(\epsilon_n {\sigma })$$
