...
format:   
    ...
    beamer:
        ...
        fontsize: "10pt"
    revealjs:
        output-file: time_iteration_beamer.html
...
- Given an intial guess $\mathbf{x_n}$
  ...
  - compute norm $\eta_n = |\mathbf{x_n} - \mathbf{x_{n+1}}|$
  - if $\eta_n<\eta$, stop and return $\mathbf{x_{n+1}}$
    - else, set $\mathbf{x_n} \leftarrow \mathbf{x_{n+1}}$ and continue
- Like usual, during the iterations, it is useful to look at $\epsilon_n=|F(\mathbf{x_n},\mathbf{x_n})|$ and $\lambda_n = \frac{\eta_n}{\eta_{n-1}}$
